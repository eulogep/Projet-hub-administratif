...
export type MissionSummary = Mission & { organization: { id: string; name: string; archived_at: string | null } | null };
const selection = "id, workspace_id, organization_id, title, description, status, starts_on, target_ends_on, completed_at, archived_at, organization:organizations(id, name, archived_at)";

function parse(row: unknown): MissionSummary {
  const source = row as Record<string, unknown>;
  return { ...missionSchema.parse(source), organization: source.organization as MissionSummary["organization"] };
}

export async function listMissions(workspaceId: string, archive: "active" | "archived" = "active", organizationId?: string, status?: string) {
