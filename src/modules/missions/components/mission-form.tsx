...
export function MissionForm({ action, mission, organizations, cancelHref = "/missions" }: { action: (state: MissionActionState, formData: FormData) => Promise<MissionActionState>; mission?: MissionInput; organizations: Array<{ id: string; name: string }>; cancelHref?: string }) {
  ...
  return <form ref={formRef} action={formAction} className="space-y-6" data-hydrated="false" noValidate>
    <label className="block text-sm font-medium">Organisation<select name="organization_id" defaultValue={mission?.organization_id ?? ""} className={control} aria-invalid={Boolean(state.fieldErrors?.organization_id)}><option value="">Sélectionner</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select></label>
